...
// export default AdminCategoryTable;import React, { useState, useEffect } from 'react';

import CategoryModal from '../../components/Admin/CategoryModal';
import Pagination from '../../components/commen/PaginationC';
import {
  useAddCategoryMutation,
  useFetchCategoryQuery,
  useUpdateCategoryMutation,
...
const AdminCategoryTable: React.FC = () => {
  ...
  const { data, refetch, isLoading } = useFetchCategoryQuery({ page: currentPage, limit: itemsPages });
  ...
  return (
    <div className="container mx-auto py-10 px-4">
            ...
            {isLoading ? (
              ...
              categories.map((category, index) => (
                <tr key={category._id} className="border-b border-gray-200 hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <img src={category.iconUrl} alt={`${category.name} icon`} className="w-9 h-9" />
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <button
                      onClick={() => handleEditCategoryClick(category)}
      ...
      {/* Replace the old pagination with our new Pagination component */}
      {totalPages > 1 && (
        <Pagination
