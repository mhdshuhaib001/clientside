...
export default function RealTimeBidding() {
  ...
  // Bid Handlers
  const handleBid = async (amount: number) => {
    if(productData.auctionStatus== 'sold'|| 'unsold'){
        toast.error('Live Bidding is end.');
      return;
    }
    if (!socketRef.current) {
      console.error('Socket is not initialized.');
  ...
  // Check if auction is active (not sold or unsold)
  const isAuctionActive = auctionStatus !== 'sold' && auctionStatus !== 'unsold' && !isAuctionEnded;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="space-y-4">
          ...
          {isAuctionActive ? (
            ...
            <div className="bg-red-100 border border-red-400 text-red-600 font-bold text-center p-4 rounded-lg">
              {auctionStatus === 'sold' ? 'This item has been sold' : 'Auction Ended'}
            </div>
          )}
        </div>
        <div className="space-y-4">
          ...
          <div className="bg-orange-100 rounded-lg p-4">
            <h3 className="text-lg font-semibold mb-4">Bidding History</h3>
            ...
            {bids.length > 0 ? (
              ...
              <div className="flex justify-center items-center">
                <p className="text-gray-600 text-center">No bids have been placed yet.</p>
              </div>
            )}
          </div>
          ...
          {isAuctionActive ? (
            ...
            <div className="flex justify-center items-center bg-red-100 border border-red-400 text-red-600 font-bold p-4 rounded-lg shadow-md">
              {auctionStatus === 'sold' 
                ? 'This item has been sold. Bidding is closed.'
                : 'Bidding is closed for this auction.'}
            </div>
          )}
        </div>
